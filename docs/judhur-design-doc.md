...
- **One card carries the pair, not two.** The deck is capped at 12 reviews a day; doubling it would halve the rate of new material, and the alternation *is* the thing to learn — كَتَبَ beside كَتَبَتْ teaches the pattern in a way two cards met a week apart do not. Review history is untouched, because the pair rides on the existing card rather than replacing it ([R-50]).
- **The prompt asks ONE form, drawn per presentation; both appear with the answer** *(amended v0.17 — owner correction)*. v0.16 showed both forms on the prompt, which was wrong for a reason worth recording: **a visible pair does the recall for you.** A learner who knows the root and can see two forms differing by ـَتْ can reason "root ك‑ت‑ب, so this is the verb, so it is *wrote*" without having retrieved the word at all. That is pattern-matching dressed as recognition, and a spaced-repetition schedule built on it measures the wrong thing — the interval grows because the inference is easy, not because the memory is durable.

  So the prompt shows one form with **no indication a counterpart exists** — not even a gender label, since the label is itself the hint. On reveal the asked form stays in place and its counterpart appears beneath it, both labelled, each with its own audio. Nothing moves, and the alternation is shown at the moment it teaches rather than the moment it gives the answer away.

  **`[R-53]`** Where a card has more than one surface form, the prompt MUST present exactly one and MUST NOT signal that others exist. Alternates belong with the answer.

  The draw is made once per presentation, not per render — the card re-renders on every tap, and re-drawing would flip the word under the learner mid-card.
- **Sentences and phrases use either gender, equally often.** Generated passages must not default to masculine; the generation prompt now asks for roughly even distribution and correct agreement either way. Speaking practice draws its prompted form the same way, rather than always asking for the citation form — fixing recognition and leaving production one-sided would have reintroduced the gap at the other end.
...
| One card carries the gendered pair, rather than two cards | The alternation is the thing to learn, and splitting it would halve the rate of new material against a 12-card cap (§2.3.1) |
| The prompt asks one form and hides that a counterpart exists | A visible pair lets the root plus the pattern supply the answer, so the schedule measures inference rather than memory (`[R-53]`) |
